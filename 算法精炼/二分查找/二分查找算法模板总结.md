...
先写出≥或者≤的，若≥则说明要去左边找，此时挪动的是right，直接让right = mid，与左中点配套再写left = mid+1排除mid，若是≤的则要去右边找，挪动的是left，直接让left = mid，与右中点配套，再写right = mid-1排除mid

#### 更新right和left本质就是为了排除（mid不可能直接命中target）/容纳（mid可能直接命中target）mid
